      ...
      .then((data) => {
        ...
        // If login credentials are correct, redirect to search page
          ...
          fetch(`/profile/${data.userID}`)
            .then((profile_data) => profile_data.json())
            .then((profile_data) => {
              console.log("profile for the user: ", profile_data);

              dispatch(addDisplayName(profile_data.displayName));
              dispatch(addPoints(profile_data.points));
              dispatch(addVideos(profile_data.videos));
